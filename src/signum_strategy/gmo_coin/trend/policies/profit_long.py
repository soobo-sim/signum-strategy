...
"""TrendLongProfit — 추세 롱 이익확보 정책.

━━ 이 클래스의 역할 ━━
포지션 보유 중 이익 보호 목적의 **tighten_stop 전용** 평가.
full_exit는 반환하지 않는다 — 청산은 TrendLongExit 담당.

━━ 실제 로직 위치 ━━
  evaluate() → compute_exit_signal(side="buy") → action == "tighten_stop" 인 경우만 통과

━━ tighten_stop 트리거 조건 (롱 기준) ━━
  ① RSI > rsi_overbought(75)      — 과매수 구간 진입
  ② RSI > rsi_extreme(80)         — 극단 과매수
  ③ 이익 > ATR × profit_atr_mult(2.0) — 충분한 이익 달성
  ④ EMA 기울기 0 ~ slope_weak(0.05)% — 모멘텀 둔화
  → SL을 price - ATR × tighten_stop_atr(1.0) 로 끌어올려 이익 일부 확보
...
class TrendLongProfit:
    """추세 롱 이익확보 정책. side='buy'."""
    ...
    def evaluate(
        ...
        """이익확보 트리거만 평가. full_exit 제외 — 청산은 TrendLongExit 담당."""
