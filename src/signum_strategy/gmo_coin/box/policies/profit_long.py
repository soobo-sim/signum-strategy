...
"""BoxLongProfit — 박스 롱 이익확보 정책.

박스 롱에서 이익확보 트리거:
  - 가격이 박스 중앙선을 통과하면 → SL을 entry(breakeven)로 tighten

full_exit은 BoxLongExit 담당. 이 클래스는 tighten만 담당한다.
"""
...
class BoxLongProfit:
    ...
    def evaluate(
        ...
        # 가격이 중앙선 위로 올라간 상태 + 아직 stop_tightened 아님
        if price > midpoint and not pos.stop_tightened:
            return ExitDecision(
                action="tighten",
                reason="box_long_midpoint_crossed",
                triggers={"midpoint": midpoint, "price": price},
            )

        return ExitDecision(action="hold", reason="box_long_profit_hold")
